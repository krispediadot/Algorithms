...
void makeCurve(pair<int, int> pos, int& answer, int sum) {

    if (pos.second >= m) {
        pos.second = 0;
        pos.first++;
        makeCurve(pos, answer, sum);
        return;
    }
    ...
    if (used[pos.first][pos.second] == false) {
        ...
        for (int dir = 0; dir < 4; dir++) {
            ...
            if (isValid(pos1, n, m) && isValid(pos2, n, m) && used[pos1.first][pos1.second] == false && used[pos2.first][pos2.second] == false) {
                ...
                makeCurve({pos.first, pos.second+1}, answer, sum + hardness);
    ...
    makeCurve({pos.first, pos.second+1}, answer, sum);
}
